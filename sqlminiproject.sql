...
select ibp.bidder_id,BIDDER_NAME,(NO_OF_MATCHES/NO_OF_BIDS)*100 as percentage_of_wins,BID_STATUS,count(*)
from ipl_bidder_details ibd
join ipl_bidder_points ibp
on ibd.bidder_id=ibp.bidder_id
join ipl_bidding_details i
on i.BIDDER_ID=ibp.BIDDER_ID
where BID_STATUS='won'
group by bidder_name
order by percentage_of_wins desc;
#2.	Display the number of matches conducted at each stadium with stadium name, city from the database?
select match_id,stadium_name,city,count(*) as number_of_matches
from ipl_match_schedule ims
join ipl_stadium ips
on ips.stadium_id=ims.STADIUM_ID
group by stadium_name;
#3.	In a given stadium, what is the percentage of wins by a team which has won the toss? 
Select ips.STADIUM_NAME,(sum((Case when Toss_winner=Match_winner then 1 else 0 end))/count(*) )*100 as 'percentage of wins'
...
select BID_TEAM,BIDDER_NAME,count(*) as 'Total bids'
from ipl_bidding_details ibd
join ipl_bidder_details ibp
on ibp.BIDDER_ID=ibd.BIDDER_ID
group by BID_TEAM;
#5.	Show the team id who won the match as per the win details?
select its.team_id,team_name,MATCHES_WON
from ipl_team_standings its
 ...
 select its.team_id,team_name,MATCHES_WON,MATCHES_LOST,sum(MATCHES_PLAYED) 'Total matches played'
from ipl_team_standings its
  join ipl_team it
 on it.team_id=its.TEAM_ID
 ...
 select PLAYER_ROLE,team_name,count(PLAYER_role='All-Rounder') as count_player_role
 from ipl_team_players itp
 join ipl_team it
 on it.TEAM_ID=itp.TEAM_ID
 join ipl_player ip
